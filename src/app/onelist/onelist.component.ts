...
export class OnelistComponent implements OnInit {
		...
		this.navigationSubscription = this.router.events.subscribe((e: any) => {
			if (e instanceof NavigationEnd) {
				this.userService.getCurrentUser().subscribe((user) => {
					...
					this.commentService.getCommentsForLists(this.listInfo.id).subscribe((comments) => {
	...
	ngOnInit(): void {
		...
		this.isDefault = this.listInfo.name === 'Watched' || this.listInfo.name === 'Currently watching' || this.listInfo.name === 'Plan to watch';
		this.owned = +sessionStorage.getItem('userid') === this.listInfo.isOwnedBy.id;
	...
	deleteListComment(userId: number): void {
		if (confirm('Are you sure you want to delete this comment ?')) {
			this.commentService.deleteListComment(this.listInfo.id, userId).subscribe(() => {
				this.refreshService.refresh(this.router.url);
			});
		}
	}

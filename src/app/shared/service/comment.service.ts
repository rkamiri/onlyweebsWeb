...
export class CommentService {
	...
	getCommentsForLists(listId: number): any {
		return this.httpclient.get<Comment[]>(environment.backend + '/comment/list/' + listId, httpOptions);
	}
	...
	deleteAnimeComment(animeId: number, userId: number): any {
		return this.httpclient.delete<any>(environment.backend + '/comment/anime/' + animeId + '/user/' + userId, httpOptions);
	}

	deleteArticleComment(articleId: number, userId: number): any {
		return this.httpclient.delete<any>(environment.backend + '/comment/article/' + articleId + '/user/' + userId, httpOptions);
	}

	deleteListComment(listId: number, userId: number): any {
		return this.httpclient.delete<any>(environment.backend + '/comment/list/' + listId + '/user/' + userId, httpOptions);
	}
}

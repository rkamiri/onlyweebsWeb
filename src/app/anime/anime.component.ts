...
export class AnimeComponent implements OnDestroy, OnInit {
	...
	ngOnInit(): void {
		...
		this.ratingService.getGlobalRatingOfAnAnime(this.activatedRoute.snapshot.data.anime.id).subscribe((gRate) => {
			if (isNaN(gRate)) {
				this.globalRate = 0;
			} else {
	...
	updateRating(): void {
		...
		this.ratingService.putCurrentUserRatingOfAnAnime(rating).subscribe(() => this.refreshService.refresh('/animes/' + this.anime.id));
	}
	...
	addAnimeToList(): void {
		...
		this.listService.getListByUserIdAndName(this.selectedList).subscribe((list) => {
			if (list.name === 'Watched' || list.name === 'Currently watching' || list.name === 'Plan to watch') {
				notInADefaultList = this.getAnimesFromAllLists().some((anime) => anime.id === this.anime.id);
				if (this.getAnimesFromAllLists().length === 0) {
					...
					if (notInADefaultList) {
						...
						this.removeAnimeFromListIfAlreadyIn(list, 'Currently watching');
						this.removeAnimeFromListIfAlreadyIn(list, 'Plan to watch');
					}
				}
			} else {
				this.addAnimeToListDumb(list);
	...
	removeAnimeFromListIfAlreadyIn(list: Lists, listName: string): void {
		this.listService.getListByUserIdAndName(listName).subscribe((watched) => {
			this.listService.getOneListContentByID(watched.id).subscribe((data) => {
				data.forEach((a) => {
					if (a.title === this.anime.title) {
						this.listService.deleteAnimeInList(watched.id, this.anime.id).subscribe();
					} else {
						this.addAnimeToListDumb(list);
					}
				});
	...
	getAnimesFromAllLists(): AnimeDto[] {
		...
		this.listService.getMyDefaultLists().subscribe((lists) => {
			lists.forEach((defaultList) => {
				this.listService.getOneListContentByID(defaultList.id).subscribe((listContent) => {
	...
	deleteAnimeComment(userId: number): void {
		if (confirm('Are you sure you want to delete this comment ?')) {
			this.commentsService.deleteAnimeComment(this.anime.id, userId).subscribe(() => {
				this.refreshService.refresh(this.router.url);
			});
		}
	}
